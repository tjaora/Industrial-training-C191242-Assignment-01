...
const DeleteIncome = document.getElementById("deleteI");
const DeleteExpense = document.getElementById("deleteE");
 ...
 * Task 3: Delete Entry
 */
function deleteEntryI() {
  
  for (let item of incomeList.children) {
    const budgetItems = document.querySelectorAll("#income-list > li");
    if (budgetItems.length > 0) {
      
      budgetItems[0].remove();
      break
    }
       item++;
  }
...
function deleteEntryE() {
  
  for (let item of expenseList.children){
  const budgetItems = document.querySelectorAll("#expense-list > li");
  if (budgetItems.length > 0) {
...
DeleteIncome.addEventListener("click",deleteEntryI);
DeleteExpense.addEventListener("click",deleteEntryE);
